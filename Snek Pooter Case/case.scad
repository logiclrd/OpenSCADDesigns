...
screw_length = 0.5 * 25.4;

screw_hole_diameter = 2.693;
screw_hole_inset = 4.7;
screw_hole_depth = screw_length + 0.5;

mount_hole_diameter = 3.2;
mount_hole_inset_x = 13.7;
mount_hole_inset_y = 6.2;
...
basement_height = screw_length - (first_floor_elevation + base_plate_height);
...
difference()
{
  union()
  {
    ...
    intersection()
    {
      union()
      {
        for (i = [0, 0.5, 1])
        {
          translate([0, i * (first_floor_depth - second_floor_depth), 0])
          base_design();
        }

        translate([0, 0.5 * (first_floor_depth - second_floor_depth), 0])
        cube([first_floor_width - 3, second_floor_depth - 3, 10], center = true);
      }
  ...
  for (x = [-1, 1])
    for (y = [-1, 1])
      translate([x * (first_floor_width / 2 - screw_hole_inset), y * (first_floor_depth / 2 - screw_hole_inset), -(base_plate_height + basement_height + 1)])
      rotate([0, 0, 180 / 43])
      cylinder(d = screw_hole_diameter, h = basement_height + base_plate_height + first_floor_elevation + 2, $fn = 43);

  // Mount holes
  for (x = [-1, 1])
    for (y = [-1, 1])
      translate([x * (first_floor_width / 2 - mount_hole_inset_x), y * (first_floor_depth / 2 - mount_hole_inset_y), -(base_plate_height + basement_height + 1)])
      rotate([0, 0, 0])
      cylinder(d = mount_hole_diameter, h = basement_height + base_plate_height + first_floor_elevation + 2, $fn = 46);
}
